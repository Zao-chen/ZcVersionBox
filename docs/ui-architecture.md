...
| utils/backupservice.* | 添加、备份、查询、预览、恢复、远程同步、导入、删除与重建 |
| utils/backupmonitor.* | 应用级扫描定时器，按 ID 保存指纹与重入状态 |
...
- Revision 的 committedAt 来自 git log -z --format=%h%x00%ct%x00%s。按 NUL 三元组解析并保留空 subject；使用提交者时间，以 UTC 保存、本机时区显示。不改变短哈希、日志排序或 amend 校验。
...
- AI 模型请求由 SettingsService 管理，绑定服务商和请求代次。Key、URL 或服务商变化使旧请求失效；手动模型名称不要求出现在获取列表中。Spinner 仅表达已有异步请求，不包装同步 Git 操作。
...
- BackupMonitor 仍属于应用，扫描周期仍为 1.5 秒；创建/销毁页面、筛选、刷新和滚动不重建监控。
...
默认数据仍在系统“文档”目录的 ZcVersionBox/Backup 和 ZcVersionBox/config.ini。备份 ID 仍为源路径 URL 百分号编码，仓库结构、扫描周期、配置键和命令行第一个参数的处理方式保持兼容。

Git/文件操作继续同步执行，未引入任务队列或新的备份调度器。恢复继续使用 BackupRestoreHelper，AI 提示词、15 秒同步提交说明等待及失败回退策略沿用原实现。

以下既有规则不在 UI 重构中调整：

1. git init 使用当前 Git 默认分支；预览返回 master，pull/push/重建强推仍使用 master。
2. 历史列表使用短哈希，现有编辑逻辑用完整 HEAD 判断可编辑性；列表的编辑尝试仍可能反馈“只能编辑最新的提交说明”并回填原文。
3. 自动备份仍按大小和修改时间识别变化，保留原有指纹推进、.git/build 过滤与失败处理。
4. 导入仍选择首个非 .git 条目，保留目录合并、目标覆盖及旧提交内路径的既有语义。
5. 重建仍清除历史、保留当前快照和远程地址，并尝试强推 master；失败使用原来的警告反馈。

此次没有备份引擎重写、依赖升级、真实数据迁移或自动发布。
