...
void WatchCallbackOld( WatcherWin32* pWatch ) {
	...
	do {
		...
		// ZcVersionBox: forward Modified events without opening the changed file.
		// FileInfo queries identity through a handle, which can make a concurrent
		// QSaveFile replacement fail. The application coalesces events and hashes
		// contents; size/time-only deduplication also hides same-metadata edits.
...
void WatchCallbackEx( WatcherWin32* pWatch ) {
	...
	for ( size_t i = 0; i < pWatch->ExtendedEvents.size(); i++ ) {
		...
		// See WatchCallbackOld: Modified events must not open source files.
		if ( FILE_ACTION_RENAMED_OLD_NAME == event.Action ) {
			PendingRenameWin32 pending;
			pending.FileName = nfile;
			pending.FileId = event.FileId;
			pending.CreatedAt = GetTickCount64();
